...
residents.forEach(r => {
  ...
  // Mapa de regras: qual tarefa exige morador de qual lado
  ...
  sortedTasks.forEach(task => {
    ...
    // Filtra candidatos pelo lado exigido, se a tarefa tiver essa regra
    ...
    if (requiredSide) {
      ...
      if (doLadoCerto.length > 0) {
        ...
        // Fallback de segurança: se ninguém do lado exigido estiver disponível
        // (ex: só sobrou gente do lado errado), não trava o algoritmo.
    ...
    if (selectedResident) {
      // Remove o selecionado da lista principal de disponíveis
      // (necessário porque "candidatos" pode ser um array filtrado, diferente de availableResidents)
